...
## sinfo
@app.route('/api/v1/sinfo/', methods=['GET'])
def py_sinfo():
    ...
    if output_format == "table":
        return render_template("../template/table.html", table=sinfo.to_html(index=True))
...
## sjob (based on `sacct`)
@app.route('/api/v1/sjob/<username>', methods=['GET', 'POST'])
def py_job(username):
    ...
    sjob = subprocess.run(["sacct","-u", "{}".format(username), "-S", "{}".format(startdate), "-E", "{}".format(enddate), "--state", "{}".format(status)], stdout = subprocess.PIPE) 
    ...
    return render_template("../template/table.html", table=pd_sjob.to_html(index=True))
